...
% WANT? file_sexpr(O) --> one_blank,!,file_sexpr(O).
...
%% ok_varname_or_int( ?Name) is semidet.
%
% Ok Varname.
%
...
with_lisp_translation(In,With):- 
   ...
   repeat,
     ...
     maybe_notrace(w_l_t(In,O)),
      (O== end_of_file -> ! ; (must_det(once(maybe_notrace(call(With,O)))),fail)).
